...
$tempo = time()-1209600;
$rs = mysql_query("SELECT playerID,lastUpdate from players where lastUpdate>='$tempo' and ativo<>'0' and rating <= 1800 and rating>=1501 and pontos>0 order by rating DESC,rating_month DESC $limit");
...
<?
    $n=1;
	$p=mysql_query("SELECT rating_month,firstName,playerID,rating_max,pontos,rating,lastUpdate from players where lastUpdate>='$tempo' and rating <= 1800 and rating>=1601 ativo<>'0' order by rating_month DESC,pontos DESC limit $CFG_RANKING_LIMIT");
...
$p = mysql_query("SELECT rating,firstName,playerID,rating_max,pontos,lastUpdate from players where lastUpdate>='$tempo' and ativo<>'0' and
		 rating <= 1800 and rating>=1601 and pontos>0 order by rating DESC,rating_month DESC limit $inicio,$perpage");
