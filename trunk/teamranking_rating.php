  ...
  <tr>
	...
	<table border="1">
      ...
      <tr>
        ...
        <input name="button9" class="BOTOES" type="button" style="cursor: hand" onClick="window.location='teamranking_rating.php'" value="Team Avg. Rating"></td>
      </tr>
</table>
	<BR>
	...
	<tr><th colspan=3><?=$MSG_LANG["teamranking"] ?></th>
	</tr>
	<tr>
	<th>&nbsp;</th>
...
<?
	$n=$inicio+1;
	$p = mysql_query("SELECT * from team");
 	$ratings = array();
	while ($row = mysql_fetch_array($p)){
		...
		if (mysql_num_rows($p2)>=2){
			//$time->id = $row['teamID'];
			//$time->name = $row['name'];
			//$ratings[getTeamRating($row['teamID'])] = $time;
			$ratings[$row[teamID]] = getTeamRating($row['teamID']);
		}
	}
