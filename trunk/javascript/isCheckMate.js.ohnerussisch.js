...
function isCheckMate(curColor)
{
 ...
 for(var i=-1;i<=1;i++)
 {
  for(var j=-1;j<=1;j++)
  {
   ...
   {
	 if((board[kingRow+i][kingCol+j]==0)&&(isSafe(kingRow+i,kingCol+j,curColor,true)))
    {
...
function canBlockAttack(curColor)
{
 ...
 while(getNextAttacker(kingRow,kingCol,curColor,attackerCoords))
 {
  ...
  if(canBeCaptured)
  {
   if((acord.row>-1)&&(acord.col>-1))
   {
    ...
    board[tpR][tpC]=EMPTY;
    if(!isSafe(kingRow,kingCol,curColor,false))
    {
     canBeCaptured=false;
    }
   ...
   if((canKing(kingRow,kingCol,targetKing,curColor,"capture",attackerRow,attackerCol))&&((kingRow==acord.row)&&(kingCol==acord.col)))
   {
    ...
    canBeCaptured=isSafe(attackerRow,attackerCol,curColor,true);
    board[attackerRow][attackerCol]=attacker;
   }
  }
