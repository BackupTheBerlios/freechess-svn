...
void int2oct(char *pos, int nro)
{
 int resto, quociente;

 resto=nro%8;
 quociente=floor(nro/8);
 pos[0]=resto+97;
 pos[1]=quociente+49;
 pos[2]=0;
}
...
//   FILE *in;
//   in=fopen("boardIn.txt","rb");
//   lado=fgetc(in)-48;
//   fclose(in);

//   printf("Computador: %d (0-Branco, 1-Preto)\n",lado);
/*============================*\
      Code By André End
\*============================*/
...
/*============================*\
      Code By André Start
\*============================*/
         strcpy(bestmove,MoveStr(CompMove));
/*============================*\
      Code By André Start
\*============================*/
...
\*============================*/
         FILE *out;
         out=fopen("move.txt","wb");
         fwrite(bestmove,4,1,out);
         fclose(out);
         played=1;
/*============================*\
      Code By André End
...
/*============================*\
      Code By André Start
\*============================*/
		if (!strcmp(s, "writeBoard")) { // sets castle flags to null
			WriteMyFEN();
			continue;
		}
...
\*============================*/
   ...
   char fc, sideTmv, playerSide, pMove[4], cply[2];

   fread(cply,sizeof(cply),1,in);
   cply[2]=0;
   max_depth=atoi(cply);

   playerSide=fgetc(in);
   fread(pMove,sizeof(pMove),1,in);
  ...
  for(int x = 0; x < 8; x++)
  {
   for(int y = 0; y < 8; y++)
   {
       ...
       {
        case '.':
         BOARD(y,x).color = NO_COLOR;
        break;
        case 'P':
         BOARD(y,x).color = WHITE;
         BOARD(y,x).piece = PAWN;
        break;
        case 'p':
   ...
   fread(&castle,sizeof(castle),1,in);
   fread(&enpassant,sizeof(enpassant),1,in);
   fread(&hply,sizeof(hply),1,in);
   fread(&fifty,sizeof(enpassant),1,in);
...
void WriteBoard()
{
   ...
   tmp=fopen("boardOut.txt","wb");
	...
	for(y = 7; y >= 0; y--)
	{
		...
		{
			if(BOARD(x,y).color == BLACK) { c = 1; } else { c = 0; }
			fputc(piece_char[BOARD(x,y).piece]+(c*32),tmp);
		}
		fputc('\n',tmp);
	}
   fputc('\n',tmp);
...
void WriteMyFEN()
{
   ...
   itoa(max_depth, cply, 10);
   cply[0]=(max_depth/10)+48;
   cply[1]=(max_depth%10)+48;
   cply[2]=0;

   fwrite(cply,sizeof(cply),1,tmp);

   strcpy(mvk,"@move");
   fwrite(mvk,sizeof(mvk),1,tmp);
   ...
   fwrite(&castle,sizeof(castle),1,tmp);
   fwrite(&enpassant,sizeof(enpassant),1,tmp);

   fwrite(&hply,sizeof(hply),1,tmp);
   fwrite(&fifty,sizeof(enpassant),1,tmp);
