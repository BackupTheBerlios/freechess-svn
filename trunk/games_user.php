...
$rs = mysql_query("SELECT count(*) from games WHERE whitePlayer='$player' AND gameMessage='' OR blackPlayer='$player' AND gameMessage=''"); $row = mysql_fetch_array($rs);
...
<?
    $p = mysql_query("SELECT games.*,DATE_FORMAT(dateCreated, '%d/%m/%y') as inicio,DATE_FORMAT(lastMove, '%d/%m/%y') as fim from games WHERE whitePlayer='$player' AND gameMessage='' OR blackPlayer='$player' AND gameMessage='' ORDER BY dateCreated DESC LIMIT $inicio,$perpage"); if (mysql_num_rows($p)>0){
    while ($row = mysql_fetch_array($p)){
        ...
        echo "<tr>
                <td align='center'><a href='stats_user.php?cod=$idw&voltar=games_user.php'>$white</a> x <a href='stats_user.php?cod=$idb&voltar=games_user.php'>$black</a></td> 
